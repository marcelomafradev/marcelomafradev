...
const Sidebar = () => {
  ...
  const { navigation, personalInfo } = messages as {
    personalInfo: { title: string };
    navigation: { titles: string; items: string };
  };
    ...
    <aside className="relative hidden h-screen flex-col justify-between border-r md:w-80 lg:flex">
      <div>
        ...
        <div className="flex flex-col gap-4 p-4">
          {SIDEBAR_ITEMS.map(({ items, hasArrowIcon }, index) => (
            <FadeInMotion
              key={index}
              ...
              <div className="flex flex-col">
                {items.map((item, index) => {
                  return (
                    <NavigationButton
                      {...item}
                      name={navigation.items[item.id] || item.name}
                      key={index}
                      pathname={pathname}
                      onClick={() => {
